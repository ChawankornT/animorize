...
**ทุก enum ต้องใช้ `DO $$ BEGIN ... EXCEPTION WHEN duplicate_object THEN NULL; END $$;` pattern**
เพราะ `user_role` ถูก apply ไปแล้วใน Phase 1 — CREATE TYPE ซ้ำจะ error
(PostgreSQL 15 รองรับ `CREATE TYPE IF NOT EXISTS` แต่ใช้ DO block เผื่อ PG14 ปลอดภัยกว่า)

```
user_role:      user, admin                              (Phase 1 — มีอยู่แล้ว)
media_type:     anime, series, movie, ova, special       (ไม่มี documentary — ไม่อยู่ใน DECISIONS.md)
...
```sql
DO $$ BEGIN
  CREATE TYPE media_type AS ENUM ('anime', 'series', 'movie', 'ova', 'special');
...
provider_id     UUID FK → providers(id) ON DELETE SET NULL nullable
                -- NOTE: ไม่ enforce FK กับ media_providers (validated at application layer)
                -- user อาจเลือก provider ที่ไม่ได้ assign ให้ media นั้น — เช็คใน usecase แทน
audio           audio_type default 'sub'
...
สร้าง `is_admin()` helper function — **ต้องใส่ STABLE** เพื่อให้ PG cache ผลใน 1 transaction (ไม่ query profiles ทุก row ที่ RLS เช็ค):
```sql
...
- **Fresh DB:** รัน 00-12 ทั้งหมดตามลำดับ
- **Existing Phase 1 DB (มี profiles + user_role แล้ว):** รัน 01-12 ตามลำดับได้เลย — enums ใช้ `DO/EXCEPTION` pattern จึงไม่ error ถ้า type มีอยู่แล้ว, 02-profiles.sql skip ได้ (มีอยู่แล้ว)
- ระบุว่าไฟล์ไหนเป็น Phase 2 only (03-12) และไฟล์ไหนเป็น Phase 1 (00-02)
...
- ห้ามใส่ `documentary` ใน media_type — ไม่อยู่ใน DECISIONS.md (เพิ่มทีหลังได้ด้วย ALTER TYPE ADD VALUE)
- ห้ามลืม STABLE ใน is_admin() function
