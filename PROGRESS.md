...
- **In progress:** Phase 5 — UX Polish — **P0 pre-phase docs ✅** (merged to `develop`, PR #29, 2026-08-26); full spec = 16 decisions in `DECISIONS.md` §P5; **P1 (DS chore) implemented on `feature/phase5-p1-ds-chore` (2026-08-27)** — `Popover` + `Modal` gap-fill + shared `ToastPortal` + doc sync done, lint/typecheck/build/tests all green, PR not yet opened; next up: manual smoke test (owner) → open PR → P2/P3
...
| P1   | DS chore — `Popover` component ใหม่ · `Modal` → native `<dialog>` (rescoped — verify/gap-fill only, not full conversion) · shared `ToastPortal` (ยุบ 3 duplicate) + BRAND/DECISIONS/CLAUDE.md doc-sync | implemented on `feature/phase5-p1-ds-chore` (2026-08-27) — lint/typecheck/build/test green, manual smoke test + PR pending |
| P2   | Edit progress + `⋯ More` menu + Back button                                                                                                                                                            | blocked by P1 only — design addendum ✅ delivered (2026-08-27)                                                             |
...
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-27 | feat(phase5): P1 DS chore — new `Popover` (roving-tabindex a11y, 7 tests) · `Modal` gap-fill (dark rule + `AddToLibraryModal` migration) · shared `ToastPortal` (3 duplicates → 1, standardized position/z-index) · `Toast.tsx` role-per-variant fix · BRAND/DECISIONS/CLAUDE.md doc sync — on `feature/phase5-p1-ds-chore`, PR not yet opened                                                                      | `Popover.tsx`, `ToastPortal.tsx` (new); `Modal.tsx`, `Toast.tsx`, `AddToLibraryModal.tsx`, `FavoriteButton.tsx`, `EpisodeTracker.tsx`, `globals.css`, `dev/components/page.tsx`; `BRAND.md`, `DECISIONS.md`, `CLAUDE.md`, `PROGRESS.md` |
...
### Phase 5 — P1 implementation (2026-08-27) — on `feature/phase5-p1-ds-chore`, not yet merged
...
- Verification: `npm run lint` ✅ (0 errors, 1 pre-existing unrelated warning in `useToast.ts`) · `npx tsc --noEmit` ✅ · `npx vitest run` ✅ 157/157 (150 baseline + 7 new) · `npm run build` ✅. **Manual smoke test (keyboard/Esc/backdrop in an actual browser) not done by Claude Code** — no browser-automation tool was available in this session; owner checking manually at `localhost:3000/dev/components` before PR opens.
...
- **PR not opened yet** — branch is `feature/phase5-p1-ds-chore`, based on `develop` at `389f88a` (which itself is the P1-prep-session docs commit, `chore(phase5): P1 prep session…`, committed separately before branching).
