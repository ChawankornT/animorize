...
import { createMockUserMediaRepository, makeUserMediaWithMedia } from '@/__tests__/utils/mockRepositories';
...
describe('listUserLibrary', () => {
  it('returns all items when filter=all', async () => {
    ...
    const completed = makeUserMediaWithMedia({ id: 'um-2', status: 'completed', isFavorite: false });
  ...
  it('returns only watching + favorites when filter=dashboard', async () => {
    ...
    const favCompleted = makeUserMediaWithMedia({ id: 'um-2', status: 'completed', isFavorite: true });
    const plain = makeUserMediaWithMedia({ id: 'um-3', status: 'plan_to_watch', isFavorite: false });
    const repo = createMockUserMediaRepository();
  ...
  it('defaults to filter=all when no filter specified', async () => {
    ...
    repo.findByUserId = vi.fn().mockResolvedValue([
      makeUserMediaWithMedia({ status: 'completed', isFavorite: false }),
    ]);
